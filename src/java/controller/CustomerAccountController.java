        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("changeCustomerProfile")) {
                ...
                if (submit == null) {
                    ...
                    if (Name.isEmpty() || Address.isEmpty() || PhoneNum.isEmpty() || YearOfBirth.isEmpty()) {
                    ...
                    Account acc = new Account(cid, Name, PhoneNum, Address, YearOfBirtH);
                    int updated = dao.updateCustomer(acc);

                    if (updated > 0) {

                        // Cập nhật lại session với thông tin mới nhất
                        Account updatedCustomer = dao.getAccountById(customer.getAccountID()); // Lấy dữ liệu mới từ DB
                        session.setAttribute("account", updatedCustomer); // Cập nhật lại session
